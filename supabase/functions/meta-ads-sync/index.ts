...
Deno.serve(async (request) => {
  ...
  const { data: libraries, error } = await admin
    .from("ad_libraries")
    .select("id,user_id,platform,advertiser_name,library_url,status,current_ad_count")
  ...
  const results = [];
  for (const library of (libraries ?? []) as AdLibrary[]) {
    const scraped = await scrapeMetaAdCount(library.library_url);
